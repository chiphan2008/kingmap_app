...
export default class BonusAgency extends Component {
  constructor(props){
    super(props);
    this.state={
      valCTV:'',
  ...
  searchUser(keyword){
    ...
    let url = `${global.url}${'static/find-daily-ctv'}`;
  ...
  postContent() {
    const { lang } = this.props;
    const { itemCTVChoose,rate } = this.state;
    if(itemCTVChoose.id===undefined){
      Alert.alert(lang.notify,lang.plz_user);
    }else if(rate===''){
      Alert.alert(lang.notify,lang.plz_enter_rate);
    }else {
      let url = `${global.url}${'static/change-rate'}`;
      const arr = new FormData();
      arr.append('client_id',itemCTVChoose.id);
      arr.append('rate',rate);
    ...
    return (
      ...
      <ScrollView>
        <View style={headCatStyle}>
            <View style={headContent}>
                ...
                <Image source={arrowLeft} style={{width:18, height:18,marginTop:5}} />
                  ...
                  <Text style={{marginTop:5,color:'#fff', fontWeight: '600'}}>{lang.bonus_ctv.toUpperCase()}</Text>
                <View></View>
            </View>
        </View>
         ...
         <View>
           ...
           <View style={wrapWhite}>
             <View style={{flexDirection:'row',alignItems:'center'}}>
                <Text numberOfLines={1} style={[colorTitle, {fontWeight: 'bold', fontSize:14}]}>{`${lang.ratio}`}</Text>
                <TextInput underlineColorAndroid='transparent'
                style={{marginLeft:10,width:width-150,backgroundColor:'#EDEDED',padding:5}}
        ...
        <View style={{alignItems:'center',marginTop:10,marginBottom:height/3}}>
          ...
          <Text style={{color:'#fff', fontWeight: 'bold',fontSize:18}}>{`${lang.update}`}</Text>
          </TouchableOpacity>
        </View>
            ...
            <FlatList
             ...
             renderItem={({item,index}) =>(
               <TouchableOpacity onPress={()=>{
                 this.setState({itemCTVChoose:item,rate:item.rate_revenue.toString(),noDataUser:'',listUser:[]});
               }}
