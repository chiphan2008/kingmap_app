    ...
    if(responseJson.code===200){
      getEncodeApi(`${global.url_node}${'person/'}${responseJson.data[0].id}`).then(e=>{
        if(e.data===undefined || e.data.length===0) {
          //console.log(`${global.url_node}${'person/add'}`,'POST',responseJson.data[0]);
          encodeApi(`${global.url_node}${'person/add'}`,'POST',responseJson.data[0]);
        }else {
          encodeApi(`${global.url_node}${'person/update'}`,'POST',responseJson.data[0]);
        }
      })
